...
class NTiFileParser(AbstractNoiseParser):
    ...
    def parse(self, file_path: str, return_all_columns: bool = False) -> ParsedData:
        ...
        # FIX 1: More robust condition to avoid filtering out valid "...rpt_report.txt" files.
        if "_report.txt" in filename_lower and "rpt_report" not in filename_lower:
        ...
        try:
            ...
            # FIX 2: More flexible marker detection. Searches for multiple possible headers.
            if is_spectral:
                markers_to_try = ["# RTA LOG Results", "# RTA Results"]
            else: # Broadband
                markers_to_try = ["# Broadband LOG Results", "# Broadband Results"]

            table_start_idx = -1
            found_marker = None
            for marker in markers_to_try:
                idx = next((i for i, l in enumerate(content_lines) if l.strip().startswith(marker)), -1)
                if idx != -1:
                    table_start_idx = idx
                    found_marker = marker
                    logger.debug(f"Found data marker '{found_marker}' at line index {table_start_idx}")
                    break

            if table_start_idx == -1:
                err_msg = f"Could not find any of the expected data markers: {markers_to_try}"
                parsed_data_obj.metadata['error'] = err_msg
                logger.error(f"{err_msg} in file {file_path}")
                return parsed_data_obj
