...
- Layer responsibilities (event handlers → thunks → actions → reducers → selectors/renderers)
...
```
User Interaction → Event Handler → Thunk → Action → Reducer → State Update
                                                                    ↓
                                                            onStateChange
                                                                    ↓
...
3. **Strict Layer Separation:** Logic must go in the correct layer (see AGENTS.md §2)
...
**3. Auto-Export Mode (Live + Optional Background Export)**
- **Disabled by default** - no static HTML is generated unless explicitly requested
- Add `--create-static` flag to generate static HTML in background thread while live server runs
- Example: `bokeh serve noise_survey_analysis --show --args --create-static`
...
- Auto-generates config for quick reloading
- **Static HTML export is disabled by default** - add `--create-static` to generate
- Best for: First-time use, exploring new datasets

#### Data Caching System

**Prepared-data caches:**
- Lightweight initial `DataManager` instances are cached in
  `noise_survey_datamanager_cache.pkl`; every session receives a deep copy so
  streaming mutations cannot contaminate another session.
- Completed audio-alignment tables are cached separately in
  `noise_survey_audio_alignment_cache.pkl`. This avoids reloading deferred log
  arrays and rerunning bounded WAV/log correlation merely because a new browser
  session was opened.
- The audio-alignment cache key fingerprints source paths, file sizes,
  modification times, audio-directory contents, source options and alignment
  settings. It invalidates automatically when any of those inputs change.
- Bokeh document models still have to be constructed per browser session; they
  cannot be shared safely between documents.
- Initial heavy work is scheduled only after the browser WebSocket connects, so
  Bokeh cannot discard a slow, nearly-built unused HTTP session and rebuild it.
...
**Cache Invalidation:**
- Parsed-file and audio-alignment caches invalidate automatically when a source
  file's size or modification time changes.
- Manual reset: delete `noise_survey_datamanager_cache.pkl`,
  `noise_survey_audio_alignment_cache.pkl`, and (when a true reparse is needed)
  the `noise_survey_parsed_cache` directory from the temp directory.
...
# Quick start with config AND generate static HTML in background
bokeh serve noise_survey_analysis --show --args --config .\config.json --create-static

# Resume saved analysis session
bokeh serve noise_survey_analysis --show --args --workspace .\my_analysis.json

# Explore new dataset interactively (no static export)
bokeh serve noise_survey_analysis --show

# Explore new dataset AND generate static HTML after selection
bokeh serve noise_survey_analysis --show --args --create-static

# Generate static HTML only (no live server)
python -m noise_survey_analysis.main --generate-static .\config.json
...
- [ ] Verify naming conventions (see AGENTS.md §3)
...
2. ❌ Business logic in event handlers or renderers
3. ❌ Dispatching actions from renderers
4. ❌ Accessing `registry.models` directly in thunks
5. ❌ Changing Bokeh image buffer dimensions after initialization
6. ❌ Forgetting to update manual test checklist for UI changes
7. ❌ Hardcoding values that should be constants
