...
def scan_directory_for_sources(directory_path, auto_group=True):
    ...
    patterns = {
        '*.csv': None,  # CSV files need additional checks to determine parser type
        '*.xls': 'svan',
        '*.xlsx': 'svan',
        '*_Rpt_Report.txt': 'nti',
    ...
    for root, _, files in os.walk(directory_path):
        for pattern, parser_type in patterns.items():
            ...
            for filename in fnmatch.filter(files, pattern):
                ...
                if parser_type is None and filename.lower().endswith('.csv'):
                    # Check for Sentry date-time pattern in filename
                    # Example: 5879 Land At Redhayes, Exeter_2025_04_29__17h55m29s_2025_04_24__12h10m00s.csv
                    if re.search(r'_\d{4}_\d{2}_\d{2}__\d{2}h\d{2}m\d{2}s_\d{4}_\d{2}_\d{2}__\d{2}h\d{2}m\d{2}s', filename):
                        current_parser_type = 'sentry'
                    else:
                        current_parser_type = 'svan'
                else:
                    current_parser_type = parser_type
                ...
                elif current_parser_type in ['sentry', 'svan']:
                    data_type = 'overview'  # Default for sentry/svan
                elif current_parser_type == 'audio':
                    data_type = 'audio'

                source_entry = {
                    "position_name": position_name_guess,
                    "file_path": full_path,
                    "parser_type": current_parser_type,
                    "data_type": data_type,
                    "filename": filename,
